...
SWEP.Positions = {
    {
        pos = Vector(0, 0, 0),
        ang = Vector(0, 0, 0)
    }, -- Viewmodel positions in IDLE mode
    {
        pos = Vector(0, 0, 0),
        ang = Vector(0, 0, 0)
    }, -- Viewmodel positions in AIM mode
    {
        pos = Vector(0, 0, 0),
        ang = Vector(0, 0, 0)
    } -- Viewmodel positions in SPRINT mode
}
...
function SWEP:Reload()
    if self.NextReload > CurTime() or self.Owner:GetAmmoCount(self.Primary.Ammo) <= 0 or self:GetNWMode() == MODE_SPRINT or self:GetNWMode() == MODE_AIM or not IsFirstTimePredicted() then return end
...
function SWEP:Think()
    if CLIENT and IsValid(self) then
        ...
        if self:GetNWMode() == MODE_SPRINT and (not self.Owner:KeyDown(IN_SPEED) or speed < self.Owner:GetWalkSpeed()) then
            self:SetNWMode(MODE_NORMAL)
        end
...
hook.Add("KeyRelease", "jbWepBaseHandleUnAim", function(p, k)
    if IsValid(p) and k and k == IN_ATTACK2 and not cvarAimToggle:GetBool(p) then
