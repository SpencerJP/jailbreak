...
function ENT:Think()
    -- there is no registered transmitter and yet we're here. What's going on? Let's assume that we are the transmitter that is being looked for.
    if not IsValid(JB.TRANSMITTER) and IsValid(self) then
        JB.TRANSMITTER = self
    end
end
...
function ENT:KeyValue(key, value)
    if (self:SetNetworkKeyValue(key, value)) then return end
end
