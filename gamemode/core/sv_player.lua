...
JB.Gamemode.PlayerSpawn = function(gm, ply)
    if (ply:Team() ~= TEAM_PRISONER and ply:Team() ~= TEAM_GUARD) or (not ply._jb_forceRespawn and (JB.State == STATE_LASTREQUEST or JB.State == STATE_PLAYING or (JB.State ~= STATE_IDLE and CurTime() - JB.RoundStartTime > 10))) then
        ply:KillSilent()
...
JB.Gamemode.PlayerDeathThink = function(gm, ply)
    if (ply:KeyPressed(IN_ATTACK) or ply:KeyPressed(IN_ATTACK2) or ply:KeyPressed(IN_JUMP)) and ply:GetObserverMode() == OBS_MODE_NONE then
...
JB.Gamemode.PlayerCanPickupWeapon = function(gm, ply, entity)
    ...
    if entity:GetClass() == "weapon_physgun" then return ply:IsSuperAdmin() end
    if not ply:CanPickupWeapon(entity) then return false end
    if entity.IsDropped and (not entity.BeingPickedUp or entity.BeingPickedUp ~= ply) then return false end

    if JB:CheckWeaponReplacements(ply, entity) then
        entity:Remove()
...
JB.Gamemode.PlayerShouldTakeDamage = function(gm, a, b)
    if IsValid(a) and IsValid(b) and a:IsPlayer() and b:IsPlayer() and a:Team() == b:Team() and (JB.State == STATE_SETUP or JB.State == STATE_PLAYING or JB.State == STATE_LASTREQUEST) and (not IsValid(JB.TRANSMITTER) or a:Team() ~= TEAM_PRISONER or not JB.TRANSMITTER:GetJBWarden_PVPDamage()) then return false end

    return true
end

JB.Gamemode.IsSpawnpointSuitable = function() return true end
...
JB.Gamemode.PlayerDeath = function(gm, victim, weapon, killer)
    ...
    if IsValid(killer) and killer.IsPlayer and killer:IsPlayer() and killer:Team() == TEAM_PRISONER and victim:Team() == TEAM_GUARD and killer.AddRebelStatus and not killer:GetRebel() and tonumber(JB.Config.rebelSensitivity) >= 1 and JB.State ~= STATE_LASTREQUEST then
    ...
    if JB.State == STATE_LASTREQUEST then
        local guard, _prisoner = unpack(JB.LastRequestPlayers)
...
JB.Gamemode.PlayerCanHearPlayersVoice = function(gm, listener, talker)
    if (not talker:Alive()) or (talker:Team() == TEAM_PRISONER and ((CurTime() - JB.RoundStartTime) < 30)) then return false, false end
    if (talker.GetWarden and talker:GetWarden()) then return true, false end
...
hook.Add("PlayerHurt", "JB.PlayerHurt.MakeRebel", function(victim, attacker)
    if not IsValid(attacker) or not IsValid(victim) or not attacker:IsPlayer() or not victim:IsPlayer() or tonumber(JB.Config.rebelSensitivity) ~= 2 then return end

    if attacker:Team() == TEAM_PRISONER and victim:Team() == TEAM_GUARD and attacker.SetRebel and not attacker:GetRebel() and JB.State ~= STATE_LASTREQUEST then
        attacker:AddRebelStatus()
    end
end)

local painSounds = {"vo/npc/male01/ow01.wav", "vo/npc/male01/ow02.wav", "vo/npc/male01/pain01.wav", "vo/npc/male01/pain02.wav", "vo/npc/male01/pain03.wav", "vo/npc/male01/pain04.wav", "vo/npc/male01/pain05.wav", "vo/npc/male01/pain06.wav", "vo/npc/male01/pain07.wav", "vo/npc/male01/pain08.wav", "vo/npc/male01/pain09.wav"}
...
function JB.Gamemode:PlayerUse(ply, ent)
    if not ply:Alive() or not (ply:Team() == TEAM_GUARD or ply:Team() == TEAM_PRISONER) then return false end
