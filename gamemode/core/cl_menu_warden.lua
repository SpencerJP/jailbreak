...
function JB.MENU_WARDEN()
    ...
    if LocalPlayer().GetWarden and LocalPlayer():GetWarden() and tobool(JB.Config.wardenControl) then
        ...
        addButton(tostring(IsValid(JB.TRANSMITTER) and JB.TRANSMITTER:GetJBWarden_PVPDamage()), "Friendlyfire for prisoners", function()
        ...
        addButton(tostring(IsValid(JB.TRANSMITTER) and JB.TRANSMITTER:GetJBWarden_ItemPickup()), "Item pickup", function()
            RunConsoleCommand("jb_warden_changecontrol", "Pickup", tostring(not (IsValid(JB.TRANSMITTER) and JB.TRANSMITTER:GetJBWarden_ItemPickup())))
        end)

        addButton(tostring(IsValid(JB.TRANSMITTER) and JB.TRANSMITTER:GetJBWarden_TeamsEnabled()), "Team Colours", function()
            RunConsoleCommand("jb_warden_changecontrol", "Teams", tostring(not (IsValid(JB.TRANSMITTER) and JB.TRANSMITTER:GetJBWarden_TeamsEnabled())))
        end)

        -- if LocalPlayer():IsUserGroup('trusted') or LocalPlayer():IsUserGroup('moderator') or LocalPlayer():IsAdmin() then
        ...
        addButton(false, "Spawn Ammo Box", function()
            RunConsoleCommand("jb_warden_spawn", "AmmoBox")
        end)

        addButton(false, "Spawn Healthkit", function()
            RunConsoleCommand("jb_warden_spawn", "MedBox")
        end)

        addButton(false, "Spawn Breakable Crate", function()
            RunConsoleCommand("jb_warden_spawn", "Crate")
        end)

        addButton(false, "Spawn Blockade", function()
            RunConsoleCommand("jb_warden_spawn", "Blockade")
        end)

        addButton(false, "Spawn Ball", function()
            RunConsoleCommand("jb_warden_spawn", "Ball")
        end)

        addButton(false, "Spawn Chair", function()
            RunConsoleCommand("jb_warden_spawn", "Chair")
        end)

        addButton(false, "Spawn Watermelon", function()
            RunConsoleCommand("jb_warden_spawn", "Watermelon")
        end)

        addButton(false, "Random Trivia Question", function()
            RunConsoleCommand("jb_warden_trivia", "trivia")
        end)
