...
-- hook.Add("JailBreakStartMapvote",...) to implement your own mapvote. NOTE: Remember to return true!
function JB.Gamemode.JailBreakStartMapvote(rounds_passed, extentions_passed)
    -- return true in your own mapvote function, else there won't be a pause between rounds!
    return false
end
...
-- You can call this from your own admin mod/mapvote if you want to extend the current map.
function JB:Mapvote_ExtendCurrentMap()
    JB.RoundsPassed = 0
...
-- You can call this from your admin mod/mapvote to initiate a mapvote.
function JB:Mapvote_StartMapVote()
...
function JB:NewRound(rounds_passed)
    ...
    if SERVER then
        ...
        net.Start("JB.SendRoundUpdate")
        net.WriteInt(STATE_SETUP, 8)
        net.WriteInt(rounds_passed, 32)
        net.Broadcast()
...
function JB:EndRound(winner)
    ...
    if SERVER then
        if JB.RoundsPassed >= tonumber(JB.Config.roundsPerMap) and JB:Mapvote_StartMapVote() then return end -- Halt the round system; we're running a custom mapvote!
        ...
        net.Broadcast(p)
        net.Start("JB.SendRoundUpdate")
        net.WriteInt(STATE_ENDED, 8)
        net.WriteInt(winner or 0, 8)
        net.Broadcast()
    elseif CLIENT then
...
elseif SERVER then
    timer.Create("JBRoundEndLogic", 1, 0, function()
        if JB.State == STATE_IDLE and wantStartup and #team.GetPlayers(TEAM_GUARD) >= 1 and #team.GetPlayers(TEAM_PRISONER) >= 1 then
...
JB._IndexCallback.State = {
    get = function() return IsValid(JB.TRANSMITTER) and JB.TRANSMITTER.GetJBState and JB.TRANSMITTER:GetJBState() or STATE_IDLE end,
    set = function(state)
...
JB._IndexCallback.RoundsPassed = {
    get = function() return IsValid(JB.TRANSMITTER) and JB.TRANSMITTER.GetJBRoundsPassed and JB.TRANSMITTER:GetJBRoundsPassed() or 0 end,
    set = function(amount)
        if SERVER and IsValid(JB.TRANSMITTER) then
...
JB._IndexCallback.RoundStartTime = {
    get = function() return IsValid(JB.TRANSMITTER) and JB.TRANSMITTER.GetJBRoundStartTime and JB.TRANSMITTER:GetJBRoundStartTime() or 0 end,
    set = function(amount)
        if SERVER and IsValid(JB.TRANSMITTER) then
...
JB._IndexCallback.LastRequest = {
    get = function() return (JB.State == STATE_LASTREQUEST) and JB.TRANSMITTER:GetJBLastRequestPicked() or "0" end,
    set = function(tab)
        ...
        if not tab or type(tab) ~= "table" or not tab.type or not JB.ValidLR(JB.LastRequestTypes[tab.type]) or not IsValid(tab.prisoner) or not IsValid(tab.guard) then
            ...
            if not pcall(function()
                JB:DebugPrint("Attempted to select invalid LR: ", tab.type, " ", tab.prisoner, " ", tab.guard, " ", type(tab))
            end) then
                JB:DebugPrint("Unexptected LR sequence abortion!")
            end
        ...
        chainState(STATE_LASTREQUEST, 180, function()
            JB:EndRound()
        end)
...
JB._IndexCallback.LastRequestPlayers = {
    get = function() return JB.State == STATE_LASTREQUEST and {JB.TRANSMITTER:GetJBLastRequestGuard() or NULL, JB.TRANSMITTER:GetJBLastRequestPrisoner() or NULL} or {NULL, NULL} end,
    set = function()
