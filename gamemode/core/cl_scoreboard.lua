...
vgui.Register("JBScoreboard.PlayerRow", {
    ...
    OnMouseReleased = function(self)
        if LocalPlayer():IsAdmin() then
            ...
            m:AddOption("Force Swap", function()
                RunConsoleCommand("jb_admin_swap", self.Player:SteamID() or "0")
            end)

            m:AddOption("Make Spectator", function()
                RunConsoleCommand("jb_admin_swap_spectator", self.Player:SteamID() or "0")
            end)

            m:AddOption("Revive", function()
                RunConsoleCommand("jb_admin_revive", self.Player:SteamID() or "0")
            end)

            m:AddOption("Open Steam Profile", function()
                RunConsoleCommand("jb_admin_opensteamprofile", self.Player:SteamID64() or "0")
            end)

            m:AddOption("Print Steam Friends", function()
                RunConsoleCommand("jb_admin_friends", self.Player:SteamID() or "0")
            end)

            m:AddOption("Toggle Admin Disguise", function()
                JB:ToggleDisguiseMode(LocalPlayer())
            end)
...
vgui.Register("JBScoreboard.PlayerRow.Spectator", {
    ...
    OnCursorEntered = function(self)
        ...
        self.namePanel.Think = function(this)
            if not IsValid(self) or not self:IsVisible() or not IsValid(scoreboard) or not scoreboard.Expand or not scoreboard:IsVisible() or (IsValid(this) and not IsValid(self.namePanel)) then
                this:Remove()
            end
        end
...
vgui.Register("JBScoreboard", {
    Init = function(self)
        ...
        self.Host:SetText("www.reduxservers.com")
        ...
        self.Credit:SetText("www.reduxservers.com") -- don't be a douche and remove my name here
