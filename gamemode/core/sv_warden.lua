		...
		if tobool(val) then
			for k, v in pairs( team.GetPlayers(TEAM_PRISONER) ) do
				if v:IsValid() then
					...
					v:SetWalkSpeed(v:GetWalkSpeed()/2)
					v:SetRunSpeed(v:GetRunSpeed()/2)
...
concommand.Add("jb_warden_trivia",function(p,c,a)
	...
	elseif opt == "trivia" then
		...
		p:PrintMessage( HUD_PRINTTALK, "The secret answer is: " .. rnd.answer .. ". Only you know this!")
		p.nextTrivia = true
		timer.Simple(10, function() p.nextTrivia = false end)
	end
end);
