...
local randomQuestions = {
    {
        {
            question = "Who created the first version of Jail Break for Garry's Mod?",
            answer = "excl"
        },
        {
        ...
        {
            question = "Who is the person in charge of the prison?",
            answer = "warden"
        }
    }, -- Questions about the Jail Break gamemode
    {
        ...
        {
            question = "What color is the most autistic video game hedgehog?",
            answer = "blue"
        }
    }, -- Questions about internet culture, games, etc...
    {
        {
            question = "Name a game in which two teams kick a ball around.",
            answer = "football"
        },
        {
...
elseif CLIENT then
    ...
    hook.Add("PlayerBindPress", "JB.PlayerBindPress.LR.TriviaNoSayBindsFuckYou", function(pl, bind, pressed)
        if JB.LastRequest == id and table.HasValue(JB.LastRequestPlayers, pl) and string.find(bind, "say") then return true end
    end)
